...
pi.pl - Package Installer for Perl


=head1 SYNOPSIS

  $ pi.pl area default home_perl
  $ pi.pl area init home_perl
  $ pi.pl area sync
  $ pi.pl list
  $ pi.pl install Acme-Buffy
  $ pi.pl list
...
shell script: B<pi.pl>.  The shell script implementation has limited
features and it is just a proof of concept rather than final product.

The shell script B<pi.pl> requires additional software:
...
B<pi.pl> is a simple manager for Perl binary packages. "Simple" means:
...
B<pi.pl> is also free implementation of ActiveState's PPM Perl Package
Manager and it tries to be compatible with ActiveState's packages and
repositories.

B<pi.pl> is recommended for user who want to install additional Perl modules
without access to the compiler or root account. It is especially useful in
shared hosting environment.

B<pi.pl> works with PPM package format. The PPM package has two parts: PPD
file, which contains meta-informations about package, and TAR archive, which
contains the module files.

I<TODO>: B<pi.pl> also works with PAR package format. The PAR package is Perl
...
=item B<pi.pl area add> I<area> I<archlib_path>

Adds the I<area> to configuration file and create area's settings based on
I<archlib_path>.

=item B<pi.pl area remove> I<area>

Deletes the I<area> from configuration file.

=item B<pi.pl area default> I<area>

Sets the default area to I<area>.

=item B<pi.pl area init> I<area>

Initializes the area. It create F<.pi.pl-area> directory and call B<pi.pl
area sync> command on it.

=item B<pi.pl area sync> I<area>

Synchronizes installed packages with those installed by other than B<pi.pl>
(i.e. the CPAN shell). Finds .packlist files in area location and create
B<pi.pl>'s indices.

=item B<pi.pl repo> [B<list>]

Lists the repositories that B<pi.pl> is configured to use.

=item B<pi.pl repo describe> I<repo>

Describes the repository.

=item B<pi.pl install> [B<package>|B<http://url>|B<ftp://url>]I<file.ppd>|I<file.zip>
...
=item B<pi.pl remove> I<package> [--area I<area>]
...
B<pi.pl>'s main configuration file and indices are located at F<$PERL_PI_DIR>
...
The B<pi.pl>'s package names are based on $NAME.
...
B<pi.pl> search the .packlist files for each area. The package name is based on
...
$NAME. The B<pi.pl> uses $NAME names internally so it is possible to recreate 
...
name and location path are changed if the @INC array is modified. The B<pi.pl>
...
The B<pi.pl> uses YAML as internal data format. The YAML is easier to generate
